...
namespace stdex
{
	...
	namespace details
	{
		...
		consteval bool has_relocatability(constraint_level level)
		{
			switch (level)
			{
				case constraint_level::trivial: return is_trivially_move_constructible_v<T> &&
				                                       is_trivially_destructible_v<T>;
				case constraint_level::nothrow: return is_nothrow_move_constructible_v<T> && is_nothrow_destructible_v<T>;
				case constraint_level::nontrivial: return is_move_constructible_v<T> && is_destructible_v<T>;
				case constraint_level::none: return true;
				default: return false;
			}
		}
		...
		template<class P>
		struct pointer_traits : inapplicable_traits { };

		template<class P>
			requires(requires(P ptr) { { *ptr }; })
		...
		struct contains_traits : inapplicable_traits { };
		...
		struct dispatch_traits_impl<D, tuple<Args...>> : applicable_traits
		{
			...
			template<class P>
			static typename D::return_type dispatcher(char* p, Args ... args) { return D{}(**reinterpret_cast<P*>(p), forward<Args>(args)...); }
		};
		...
		struct copy_meta
		{
			...
			constexpr explicit copy_meta(in_place_type_t<P>)
				: clone([](char* self, const char* rhs) { new(self) P(*reinterpret_cast<const P*>(rhs)); }) { }
		...
		template<class M, class... Ms>
		struct facade_meta_traits<M, Ms...>
				: facade_meta_traits_impl<
					M,
					typename facade_meta_traits<Ms...>::type> { };
		...
		template<class T>
		struct flattening_traits_impl<tuple<>, T>
		{
			using type = T;
		};
		...
		template<>
		struct flattening_traits<tuple<>>
		{
			using type = tuple<>;
		};
		...
		struct basic_facade_traits_impl<F, tuple<Ds...>> : applicable_traits
		{
			using meta_type = typename facade_meta_traits<
				...
				conditional_meta_tag<is_void_v<typename F::reflection_type> ? constraint_level::none : constraint_level::nothrow,
		...
		struct basic_facade_traits<F> : basic_facade_traits_impl<F, typename flattening_traits<typename F::dispatch_types>::type> { };
		...
		{
				using interfaces_type						= std::tuple<Ds...>;
			...
			using meta_type = facade_meta<
				typename basic_facade_traits<F>::meta_type,
				dispatch_meta<Ds>...>;

			template<class P>
			static constexpr bool applicable_pointer =
					sizeof(P) <= F::maximum_size && alignof(P) <= F::maximum_alignment && has_copyability<P>(F::minimum_copyability) &&
			...
			template<class P>
			static constexpr meta_type meta{in_place_type<P>};
		};
		...
		struct facade_traits
				: facade_traits_impl<
					F,
					typename flattening_traits<typename F::dispatch_types>::type> { };
	...
	public:
		...
		static constexpr bool HasNothrowPolyConstructor = conditional_t<
			proxiable<P, F>,
			is_nothrow_constructible<P, Args...>,
			false_type>::value;
		template<class P, class... Args>
		static constexpr bool HasPolyConstructor = conditional_t<
			proxiable<P, F>,
			is_constructible<P, Args...>,
			false_type>::value;
	...
	public:
		...
		proxy(const proxy& rhs) noexcept(HasNothrowCopyConstructor) requires(!HasTrivialCopyConstructor && HasCopyConstructor)
		{
		...
		proxy(proxy&& rhs) noexcept(HasNothrowMoveConstructor) requires(HasMoveConstructor)
		{
			if (rhs.meta_ != nullptr)
			{
				if constexpr (F::minimum_relocatability == constraint_level::trivial) { memcpy(ptr_, rhs.ptr_, F::maximum_size); }
				else { rhs.meta_->relocate(ptr_, rhs.ptr_); }
				meta_ = rhs.meta_;
				rhs.meta_ = nullptr;
			}
			else { meta_ = nullptr; }
		...
		template<class P>
		proxy(P&& ptr) noexcept(HasNothrowPolyConstructor<decay_t<P>, P>) requires(HasPolyConstructor<decay_t<P>, P>)
			: proxy(in_place_type<decay_t<P>>, forward<P>(ptr)) { }

		template<class P, class... Args>
		explicit proxy(in_place_type_t<P>, Args&&... args) noexcept(HasNothrowPolyConstructor<P, Args...>) requires(HasPolyConstructor<P, Args...>)
		{
		...
		explicit proxy(in_place_type_t<P>, initializer_list<U> il, Args&&... args) noexcept(HasNothrowPolyConstructor<P, initializer_list<U>&, Args...>) requires(HasPolyConstructor<P, initializer_list<U>&, Args...>)
			: proxy(in_place_type<P>, il, forward<Args>(args)...) { }

		proxy& operator=(nullptr_t) noexcept(HasNothrowDestructor) requires(HasDestructor)
		{
			this->~proxy();
			new(this) proxy();
			return *this;
		...
		proxy& operator=(const proxy& rhs) requires(!HasNothrowCopyAssignment && HasCopyAssignment)
		{
			proxy temp{rhs};
			swap(temp);
			return *this;
		...
		proxy& operator=(const proxy&) noexcept requires(HasTrivialCopyAssignment) = default;
		...
		proxy& operator=(proxy&& rhs) requires(!HasNothrowMoveAssignment && HasMoveAssignment)
		{
			proxy temp{move(rhs)};
			swap(temp);
			return *this;
		...
		~proxy() noexcept(HasNothrowDestructor) requires(!HasTrivialDestructor && HasDestructor) { if (meta_ != nullptr) { meta_->destroy(ptr_); } }
		...
		decltype(auto) reflect() const noexcept
			requires(!is_void_v<typename F::reflection_type>) { return static_cast<const typename F::reflection_type&>(*meta_); }
		...
		void swap(proxy& rhs) noexcept(HasNothrowMoveConstructor) requires(HasMoveConstructor)
		{
			...
			else
			{
				if (meta_ != nullptr)
				{
					...
					else { new(&rhs) proxy(move(*this)); }
				}
				else if (rhs.meta_ != nullptr) { new(this) proxy(move(rhs)); }
			}
		}

		template<class P, class... Args>
		P& emplace(Args&&... args) noexcept(HasNothrowPolyAssignment<P, Args...>) requires(HasPolyAssignment<P, Args...>)
		{
			reset();
		...
		template<class P, class U, class... Args>
		P& emplace(initializer_list<U> il, Args&&... args) noexcept(HasNothrowPolyAssignment<P, initializer_list<U>&, Args...>) requires(HasPolyAssignment<P, initializer_list<U>&, Args...>) { return emplace<P>(il, forward<Args>(args)...); }

		template<class D = typename BasicTraits::default_dispatch, class... Args>
		decltype(auto) invoke(Args&&... args) requires(details::dependent_t<Traits, D>::applicable &&
	...
	namespace details
	{
		...
		public:
			...
			sbo_ptr(const sbo_ptr&) noexcept(is_nothrow_copy_constructible_v<T>) = default;
		...
		public:
			...
			deep_ptr(const deep_ptr& rhs) noexcept(is_nothrow_copy_constructible_v<T>) requires(is_copy_constructible_v<T>)
	...
	template<class F, class T, class... Args>
	proxy<F> make_proxy(Args&&... args) { return details::make_proxy_impl<F, T>(forward<Args>(args)...); }

	template<class F, class T, class U, class... Args>
	proxy<F> make_proxy(initializer_list<U> il, Args&&... args) { return details::make_proxy_impl<F, T>(il, forward<Args>(args)...); }

	template<class F, class T>
	proxy<F> make_proxy(T&& value) { return details::make_proxy_impl<F, decay_t<T>>(forward<T>(value)); }
