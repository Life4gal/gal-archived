...
#ifndef GAL_LANG_ADDON_AST_PARSER_HPP
#define GAL_LANG_ADDON_AST_PARSER_HPP
...
namespace gal::lang::addon
{
	namespace parser_detail
	{
		...
		private:
			...
			[[nodiscard]] constexpr static matrix_type build() noexcept
			{
				...
				setter(abs, alphabet::symbol, foundation::operator_assign_name::value[0]);

				setter(abs, alphabet::symbol, foundation::operator_less_than_name::value[0]);
				setter(abs, alphabet::symbol, foundation::operator_greater_than_name::value[0]);

				setter(abs, alphabet::symbol, foundation::operator_plus_name::value[0]);
				setter(abs, alphabet::symbol, foundation::operator_minus_name::value[0]);
				setter(abs, alphabet::symbol, foundation::operator_multiply_name::value[0]);
				setter(abs, alphabet::symbol, foundation::operator_divide_name::value[0]);
				setter(abs, alphabet::symbol, foundation::operator_remainder_name::value[0]);

				setter(abs, alphabet::symbol, foundation::operator_bitwise_and_name::value[0]);
				setter(abs, alphabet::symbol, foundation::operator_bitwise_or_name::value[0]);
				setter(abs, alphabet::symbol, foundation::operator_bitwise_xor_name::value[0]);
		...
		public:
			...
			[[nodiscard]] operator ast::file_point() const noexcept { return point; }
		...
		struct operator_matcher
		{
			...
			constexpr static std::array m0{
					operator_name_type{foundation::operator_logical_or_name::value}};
			constexpr static std::array m1{
					operator_name_type{foundation::operator_logical_and_name::value}};
			constexpr static std::array m2{
					operator_name_type{foundation::operator_bitwise_or_name::value}};
			constexpr static std::array m3{
					operator_name_type{foundation::operator_bitwise_xor_name::value}};
			constexpr static std::array m4{
					operator_name_type{foundation::operator_bitwise_and_name::value}};
			constexpr static std::array m5{
					operator_name_type{foundation::operator_equal_name::value},
					operator_name_type{foundation::operator_not_equal_name::value}};
			constexpr static std::array m6{
					operator_name_type{foundation::operator_less_than_name::value},
					operator_name_type{foundation::operator_less_equal_name::value},
					operator_name_type{foundation::operator_greater_than_name::value},
					operator_name_type{foundation::operator_greater_equal_name::value}};
			constexpr static std::array m7{
					operator_name_type{foundation::operator_bitwise_shift_left_name::value},
					operator_name_type{foundation::operator_bitwise_shift_right_name::value}};
			// We share precedence here but then separate them later
			constexpr static std::array m8{
					operator_name_type{foundation::operator_plus_name::value},
					operator_name_type{foundation::operator_minus_name::value}};
			constexpr static std::array m9{
					operator_name_type{foundation::operator_multiply_name::value},
					operator_name_type{foundation::operator_divide_name::value},
					operator_name_type{foundation::operator_remainder_name::value}};
			constexpr static std::array m10{
					operator_name_type{foundation::operator_unary_not_name::value},
					operator_name_type{foundation::operator_unary_plus_name::value},
					operator_name_type{foundation::operator_unary_minus_name::value},
					operator_name_type{foundation::operator_unary_bitwise_complement_name::value}};

			using m10_type = std::tuple<foundation::operator_unary_not_name, foundation::operator_unary_plus_name, foundation::operator_unary_minus_name, foundation::operator_unary_bitwise_complement_name>;

			constexpr static std::array operators{
					foundation::operation_precedence::logical_or,
					foundation::operation_precedence::logical_and,
					foundation::operation_precedence::bitwise_or,
					foundation::operation_precedence::bitwise_xor,
					foundation::operation_precedence::bitwise_and,
					foundation::operation_precedence::equality,
					foundation::operation_precedence::comparison,
					foundation::operation_precedence::bitwise_shift,
					foundation::operation_precedence::plus_minus,
					foundation::operation_precedence::multiply_divide,
					foundation::operation_precedence::unary
			};

			static_assert(std::tuple_size_v<decltype(operators)> == static_cast<std::size_t>(foundation::operation_precedence::operation_size));
	...
	class ast_parser final : public ast::ast_parser_base
	{
	public:
		using parse_depth_type = std::size_t;

	private:
		struct scoped_parser : utils::scoped_base<scoped_parser, std::reference_wrapper<ast_parser>>
		{
			friend struct scoped_base<scoped_parser, std::reference_wrapper<ast_parser>>;

			constexpr explicit scoped_parser(ast_parser& p)
				: scoped_base{p} {}
		...
		{
			if (not foundation::name_validator::is_valid_object_name(name))
			{
				throw exception::eval_error{
	...
	public:
		...
		[[nodiscard]] ast::ast_optimizer_base& get_optimizer() override { return optimizer_; }

		/**
		 * @brief Prints the parsed ast_nodes as a tree
		 */
		void debug_print_to(foundation::string_type& dest, const ast::ast_node& node, const foundation::string_view_type prepend) const override
		{
		 ...
		 * @brief Prints the parsed ast_nodes as a tree
		 */
		[[nodiscard]] std::string debug_print(const ast::ast_node& node, const foundation::string_view_type prepend) const override
		{
			std::string ret{};
	...
	private:
		...
		[[nodiscard]] auto make_node(const foundation::string_view_type identifier, const ast::file_point prev_point, Args&&... args) const { return ast::make_node<NodeType>(identifier, ast::parse_location{filename_, {prev_point, point_}}, std::forward<Args>(args)...); }

		/**
		 * @brief Helper function that collects ast_nodes from a starting position to the top of the stack into a new AST node
		 */
		template<typename NodeType>
		void build_match(ast::ast_node::children_type::size_type match_begin, const foundation::string_view_type text = "")
		{
			bool is_deep = false;

			auto location = [&is_deep, match_begin, this]() -> ast::parse_location
			{
				// so we want to take everything to the right of this and make them children
				if (match_begin != match_stack_.size())
				{
					is_deep = true;
					return ast::parse_location{filename_, {match_stack_[match_begin]->location_begin(), point_}};
				}

				// todo: fix the fact that a successful match that captured no ast_nodes doesn't have any real start position
				return ast::parse_location{filename_, {point_, point_}};
			}();

			ast::ast_node::children_type children{};

			if (is_deep)
			{
				const auto begin_pos = match_stack_.begin() + static_cast<ast::ast_node::children_type::difference_type>(match_begin);
			...
			match_stack_.push_back(optimizer_.optimize(ast::make_node<NodeType>(text, std::move(location), std::move(children))));
		}
		...
		{
			if (read_any<foundation::keyword_comment_multi_name::left_type>())
			{
				while (not point_.finish())
				{
					if (read_any<foundation::keyword_comment_multi_name::right_type>()) { break; }
					if (not read_eol()) { ++point_; }
				}
				return true;
			}

			if (read_any<foundation::keyword_comment_single_name>())
			{
				while (not point_.finish())
				{
					if (read_symbol("\r\n"))
			...
			if (read_any<foundation::keyword_comment_annotation_name::left_type>())
			{
				while (not point_.finish())
				{
					if (read_any<foundation::keyword_comment_annotation_name::right_type>()) { break; }
					if (read_symbol("\r\n"))
					{
						point_ -= 2;
						break;
		...
		struct char_parser
		{
			...
			void parse(const value_type c, const ast::file_point point, const ast::parse_location::filename_type& filename)
			{
		...
		[[nodiscard]] bool build_number()
		{
			...
			if (not point_.finish() && parser_detail::alphabet_matcher::belong(point_.peek(), parser_detail::alphabet::floating_point))
			{
				if (read_hex())
				{
					...
					match_stack_.emplace_back(this->make_node<ast::constant_ast_node>(match, begin, parser_detail::integral_packer(match, 16)));
					return true;
				}
				...
				{
					const auto match = begin.str(point_);
					match_stack_.emplace_back(this->make_node<ast::constant_ast_node>(match, begin, parser_detail::integral_packer(match, 2)));
					return true;
				}
				...
				{
					const auto match = begin.str(point_);
					match_stack_.emplace_back(this->make_node<ast::constant_ast_node>(match, begin, parser_detail::floating_point_packer(match)));
					return true;
				}
				...
				{
					// Octal
					match_stack_.emplace_back(this->make_node<ast::constant_ast_node>(match, begin, parser_detail::integral_packer(match, 8)));
				}
				else
				{
					// Decimal
					match_stack_.emplace_back(this->make_node<ast::constant_ast_node>(match, begin, parser_detail::integral_packer(match, 10)));
				}
	...
	private:
		...
		[[nodiscard]] bool do_build_argument_list(Function&& function)
		{
			...
			build_match<ast::arg_list_ast_node>(prev_size);
			skip_whitespace(true);
			return result;
		}
	...
	public:
		...
		[[nodiscard]] bool build_lambda()
		{
			...
			// '['
			if (not build_any<foundation::keyword_lambda_name::left_type>()) { return false; }

			// capture argument list is optional
			(void)build_identifier_argument_list();

			// ']'
			if (not build_any<foundation::keyword_lambda_name::right_type>())
			{
				throw exception::eval_error{
			...
			build_match<ast::lambda_ast_node>(prev_size);
			return true;
		}
		...
		[[nodiscard]] bool build_def(const bool class_context = false, const foundation::string_view_type class_name = "")
		{
			...
			if (not build_keyword(foundation::keyword_define_name::value)) { return false; }

			if (class_context) { match_stack_.emplace_back(this->make_node<ast::id_ast_node>(class_name, point_)); }
			...
			{
				if (build_any<foundation::keyword_class_accessor_name>())
				{
					// We're now a method
					if (not build_identifier(true))
					{
			...
			if (build_any<foundation::keyword_function_guard_name>())
			{
				if (not build_operator())
				{
					throw exception::eval_error{
			...
			if (is_member_method || class_context) { build_match<ast::method_ast_node>(prev_size); }
			else { build_match<ast::def_ast_node>(prev_size); }
		...
		[[nodiscard]] bool build_if()
		{
			...
			while (true)
			{
				...
				// no more else
				if (not build_keyword(foundation::keyword_else_name::value)) { break; }
		...
		[[nodiscard]] bool build_while()
		{
			...
			build_match<ast::while_ast_node>(prev_size);
			return true;
		}
		...
		[[nodiscard]] bool build_for()
		{
			...
			if (not build_keyword(foundation::keyword_for_in_name::subtype<1>::value))
			{
				throw exception::eval_error{
			...
			build_match<ast::ranged_for_ast_node>(prev_size);
			return true;
		}
		...
		[[nodiscard]] bool build_match()
		{
			...
			auto build_case = [this]
			{
				...
				if (build_any<foundation::keyword_match_case_default_name::subtype<1>>())
				{
					if (not build_operator())
					{
						throw exception::eval_error{
					...
					build_match<ast::match_case_ast_node>(c_prev_size);
					return true;
				}

				if (build_any<foundation::keyword_match_case_default_name::subtype<2>>())
				{
					if (not build_block())
					{
						throw exception::eval_error{
					...
					build_match<ast::match_default_ast_node>(c_prev_size);
					return true;
				}
			...
			if (not build_any<foundation::keyword_match_case_default_name::subtype<0>>()) { return false; }
	...
	public:
		/**
			 ...
			 */
		[[nodiscard]] bool build_continue() { return do_build_keyword_statement<ast::continue_ast_node, foundation::keyword_continue_break_return_name::subtype<0>>(); }
		...
		/**
			 ...
			 * @throw exception::eval_error throw from build_operator()
			 */
		[[nodiscard]] bool build_break() { return do_build_keyword_statement<ast::break_ast_node, foundation::keyword_continue_break_return_name::subtype<1>>(); }
		...
		/**
			 ...
			 * @throw exception::eval_error throw from build_operator()
			 */
		[[nodiscard]] bool build_return() { return do_build_keyword_statement<ast::return_ast_node, foundation::keyword_continue_break_return_name::subtype<2>>(); }
		...
		[[nodiscard]] bool build_try()
		{
			...
			if (not build_keyword(foundation::keyword_try_catch_finally_name::subtype<0>::value)) { return false; }
			...
			while (true)
			{
				...
				if (not build_keyword(foundation::keyword_try_catch_finally_name::subtype<1>::value)) { break; }
				...
				build_match<ast::try_catch_ast_node>(catch_prev_size);
			}

			while (build_eol()) {}

			if (build_keyword(foundation::keyword_try_catch_finally_name::subtype<2>::value))
			{
				...
				build_match<ast::try_finally_ast_node>(finally_prev_size);
			}

			build_match<ast::try_ast_node>(prev_size);
			return true;
		}
		...
		[[nodiscard]] bool build_class(const bool class_allowed)
		{
			...
			if (not build_keyword(foundation::keyword_class_name::value)) { return false; }
			...
			build_match<ast::class_decl_ast_node>(prev_size);
			return true;
		}
						...
						if (p.saw_interpolation_marker)
						{
							if (b.peek() == '{')
							{
								...
								if (p.is_interpolated)
								{
									...
									build_match<ast::binary_operator_ast_node>(prev_size, foundation::operator_plus_name::value);
								}
								...
								if (b.peek() == '}')
								{
									...
									match_stack_.emplace_back(this->make_node<ast::id_ast_node>(foundation::operator_to_string_name::value, begin));
									...
									build_match<ast::arg_list_ast_node>(eval_size);
									build_match<ast::fun_call_ast_node>(to_string_size);
									build_match<ast::binary_operator_ast_node>(prev_size, foundation::operator_plus_name::value);
								}
								else
								{
		...
		[[nodiscard]] bool build_symbol(const foundation::string_view_type symbol, const bool disallow_prevention = false)
		{
			...
			{
				if (point_.peek() != foundation::operator_assign_name::value[0] && parser_detail::operator_matcher::match(begin.str(point_)) && not parser_detail::operator_matcher::match(begin.str(point_ + 1)))
				{
		...
		[[nodiscard]] bool build_reference()
		{
			...
			if (build_symbol("&"))
			{
				...
				build_match<ast::reference_ast_node>(prev_size);
				return true;
			}
			return false;
		}
		...
		[[nodiscard]] bool build_inline_container()
		{
			...
			if (not build_any<foundation::keyword_inline_container_gen_name::left_type>()) { return false; }

			(void)build_container_argument_list();

			if (not build_any<foundation::keyword_inline_container_gen_name::right_type>())
			{
				throw exception::eval_error{
		...
		[[nodiscard]] bool build_map_pair()
		{
			...
			if (not build_any<foundation::keyword_pair_splitter_name>())
			{
				point_ = begin;
			...
			build_match<ast::map_pair_ast_node>(prev_size);
			return true;
		}
		...
		void builder(const auto prev_size)
		{
			...
			this->build_match<ast::unary_operator_ast_node>(prev_size, T::value);
		}
		...
		[[nodiscard]] bool build_dot_fun_call()
		{
			...
			while (true)
			{
				...
				else if (build_any<foundation::array_access_interface_name::left_type>())
				{
					...
					build_match<ast::array_access_ast_node>(prev_size);
				}
				// object.method
				else if (build_symbol("."))
				{
					...
					build_match<ast::dot_access_ast_node>(prev_size);
				}
				else if (build_eol())
				{
					const auto begin = --point_;
		...
		[[nodiscard]] bool build_var_decl(const bool class_context = false, const foundation::string_view_type class_name = "unknown_class")
		{
			...
			if (build_keyword(foundation::keyword_variable_declare_name::value))
			{
				if (class_context)
				{
					match_stack_.emplace_back(this->make_node<ast::id_ast_node>(class_name, point_));
					if (not build_identifier(true))
					{
						throw exception::eval_error{
					...
					build_match<ast::member_decl_ast_node>(prev_size);
					return true;
				}
				...
				else if (build_identifier(true)) { build_match<ast::var_decl_ast_node>(prev_size); }
				else { throw exception::eval_error{"Incomplete variable declaration", filename_, point_}; }
				return true;
			}

			if (build_keyword(foundation::keyword_global_name::value))
			{
				...
				build_match<ast::global_decl_ast_node>(prev_size);
				return true;
			}
		...
		[[nodiscard]] bool build_operator(const parser_detail::operator_matcher::group_id_type group_id = parser_detail::operator_matcher::group_ids[0])
		{
			...
			if (parser_detail::operator_matcher::operators[group_id] == foundation::operation_precedence::unary) { return build_value(); }
			...
			while (read_operator(group_id, op))
			{
				...
				{
						using enum foundation::operation_precedence;
					case logical_or:
					{
						build_match<ast::logical_or_ast_node>(prev_size, op);
						break;
					}
					case logical_and:
					{
						build_match<ast::logical_and_ast_node>(prev_size, op);
						break;
					}
					case bitwise_or:
					case bitwise_xor:
					...
					{
						build_match<ast::binary_operator_ast_node>(prev_size, op);
						break;
					}
					case unary:
					case operation_size:
		...
		[[nodiscard]] bool build_equation()
		{
			...
			if (build_operator())
			{
				...
				for (constexpr std::array operators{
						     ...
						     operator_name_type{foundation::operator_minus_assign_name::value},
						     operator_name_type{foundation::operator_multiply_assign_name::value},
						     operator_name_type{foundation::operator_divide_assign_name::value},
						     operator_name_type{foundation::operator_remainder_assign_name::value},
						     operator_name_type{foundation::operator_bitwise_shift_left_assign_name::value},
						     operator_name_type{foundation::operator_bitwise_shift_right_assign_name::value},
						     operator_name_type{foundation::operator_bitwise_and_assign_name::value},
						     operator_name_type{foundation::operator_bitwise_or_assign_name::value},
						     operator_name_type{foundation::operator_bitwise_xor_assign_name::value}};
				     const auto& op: operators)
				{
					if (build_symbol(op, true))
					{
						...
						build_match<ast::equation_ast_node>(prev_size, op);
						return true;
					}
				}
				return true;
		...
		[[nodiscard]] bool build_block()
		{
			...
			if (build_any<foundation::keyword_block_begin_name>())
			{
	...
	private:
		...
		[[nodiscard]] ast::ast_node_ptr parse_internal(const foundation::string_view_type input, const foundation::string_view_type filename)
		{
			...
			if (build_statements(true))
			{
				...
				build_match<ast::file_ast_node>(0);
			}
			else { match_stack_.emplace_back(ast::make_node<ast::noop_ast_node>()); }

			gal_assert(match_stack_.size() == 1);
			return std::move(std::exchange(match_stack_, ast::ast_node::children_type{}).front());
		}
		...
		[[nodiscard]] ast::ast_node_ptr parse_instruct_eval(const foundation::string_view_type input)
		{
			...
			auto last_match_stack = std::exchange(match_stack_, ast::ast_node::children_type{});
	...
	public:
		explicit ast_parser(const std::size_t max_parse_depth = 512)
...
#endif // GAL_LANG_ADDON_AST_PARSER_HPP
