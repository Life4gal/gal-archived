...
namespace gal::lang::foundation
{
	namespace functor_maker_detail
	{
		...
		struct function_invoker
		{
			...
			Result do_invoke(const parameters_view_type params, const type_conversion_state& conversion)
			{
				...
				else if constexpr (std::is_same_v<Result, void>) { return dispatch(functions, params, conversion); }
				else { return boxed_cast<Result>(dispatch(functions, params, conversion), &conversion); }
			}
			...
			Result operator()(P&&... params)
			{
				...
				if (manager) { return do_invoke(parameters_view_type{ps}, {*manager, manager->get_conversion_saves()}); }
		...
		[[nodiscard]] std::function<Result(Params ...)> make_function_invoker(
				Result (Params ...),
				immutable_proxy_functions_type&& functions,
				const type_conversion_state* conversion) { return std::function<Result(Params ...)>(function_invoker<Result, Params...>{.functions = std::move(functions), .manager = conversion ? conversion->operator->() : nullptr}); }

		template<typename Result, typename... Params>
		consteval auto arity(Result (*)(Params ...)) noexcept { return sizeof...(Params); }
	}// namespace functor_maker_detail
	...
	namespace boxed_cast_detail
	{
		...
		struct cast_invoker<std::function<FunctionSignature>>
		{
			...
			{
				if (object.type_info().bare_equal(make_type_info<immutable_proxy_function>())) { return make_functor<FunctionSignature>(object, conversion); }
