	...
	namespace foundation
	{
		...
		namespace dispatcher_detail
		{
			...
			private:
				...
				[[nodiscard]] boxed_value do_invoke(parameters_view_type params, const type_conversion_state& conversion) const override { return dispatch(functions_, params, conversion); }
			...
			public:
				...
				variable_type& add_variable(const name_view_type name, variable_type variable)
				{
					for (auto& stack_data = recent_stack_data();
					     auto& scope: std::ranges::reverse_view(stack_data))
					{
			...
			public:
				...
				[[nodiscard]] variable_type& get_object(const name_view_type name, variable_cache_location_type& cache_location)
				{
					if (not cache_location.has_value())
					{
						...
						// Is it in the stack?
						for (auto& scope: std::ranges::reverse_view(stack_data))
						{
				 ...
				 */
				void dump_object_to(const variable_type& object, string_type& dest) const { dump_type_to(object.type_info(), dest); }

				/**
				 * @brief Dump object info.
				 */
				[[nodiscard]] string_type dump_object(const variable_type& object) const { return dump_type(object.type_info()); }
