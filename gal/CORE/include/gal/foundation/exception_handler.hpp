...
namespace gal::lang::foundation
{
	class exception_handler_base
	{
		...
		static void cast_throw(const boxed_value& object, const dispatcher_detail::dispatcher& dispatcher)
		{
			try
			{
				throw dispatcher.template boxed_cast<T>(object);
			}
	...
	template<typename... T>
	class exception_handler : public exception_handler_base
	{
	public:
		void handle(const boxed_value& object, const dispatcher_detail::dispatcher& dispatcher) override
		{
			(exception_handler_impl::cast_throw<T>(object, dispatcher), ...);
		}
	};
}// namespace gal::lang::foundation
